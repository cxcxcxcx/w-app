...
class ConfigWindow:
    def __init__(self, app, config):
        ...
        self.cbxStatus.set_active(
                self.config.login_states_dict[self.config["login_status"]])
        ...
        self.window.set_icon_from_file(self.app.get_app_icon())
        self.window.show_all()

    def auto_login_toggled(self, widget):
        self.config["login_auto_run"] = self.cbtnAutoLogin.get_active()

    def enable_proxy_toggled(self, widget):
        self.config["proxy_enable"] = self.cbtnEnableProxy.get_active()
